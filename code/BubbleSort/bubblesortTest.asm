...
arr:  	.word 1, 4, 3, 1 
	...
	.globl main
main:
	addiu $t0, $zero, 4   #load size of n
	addiu $t1, $zero, 1   #load flag. '1' means swapped
	la $t3, arr          #load base address of array(our processor doesnt support this instruction)
			     #Instead we can just load a zero by doing an addiu instruction
...
endWhile:
 	...
 	la $t7, arr	    #display the final array
 	lw $s4, ($t7)
 	lw $s5, 4($t7)
 	lw $s6, 8($t7)
 	lw $s7, 12($t7)	
